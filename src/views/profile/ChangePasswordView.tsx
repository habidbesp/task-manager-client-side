...
import { ChangeProfilePassword } from "@/types/index";
import { useMutation } from "@tanstack/react-query";
import { changeUserProfilePassword } from "@/api/ProfileAPI";
...
export default function ChangePasswordView() {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<ChangeProfilePassword>({ defaultValues: initialValues });

  const current_password = watch("current_password");
  const password = watch("password");

  const { mutate } = useMutation({
    mutationFn: changeUserProfilePassword,
    onError: (error) => {
  ...
  const handleChangePassword = (formData: ChangeProfilePassword) => {
    mutate(formData);
  };
